...
unsigned int  uiUsec;               	// 秒值
unsigned char pucUart_Buf[10];      	// 串行口缓存
...
void Uart_Proc(void)
{
  ...
  Uart_RecvString();                	// 可用中断方式实现
  if (ucUart_Num > 0x80)            	// 接收完成
  {
    if (ucUart_Num == 0x82)
      uiSec = ((pucUart_Buf[0]-'0')*10) + pucUart_Buf[1]-'0';
    ucUart_Num = 0;
  }
}
