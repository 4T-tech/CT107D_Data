...
void main(void)
{
  Close_Peripheral();
  T1_Init();
  AT24C02_ReadBuffer((unsigned char*)&ucCnt, 0, 1);
  ucCnt++;
  AT24C02_WriteBuffer((unsigned char*)&ucCnt, 0, 1);
...
void Seg_Proc(void)
{
  if (uiSeg_Dly > 300)              	// 300ms刷新1次
  {
    ...
    {
      case 0:                        	// 显示系统启动次数
        sprintf(pucSeg_Char, "1   %04u", (int)ucCnt);
        break;
      case 0x10:                    	// 显示ADC值
        ucAdc = PCF8591_Adc(3);
        sprintf(pucSeg_Char, "2    %03u", (int)ucAdc);
        PCF8591_Dac(ucAdc);
    }
